...
ribbon: black
description: "Suspected state-affiliated actor targets Afghanistani and Kazakh entities"
categories:
  - Threat Research
tags:
  - Threat Research
...
Whilst hunting for C2 infrastructure on Censys, we uncovered a suspected state-affiliated cluster targeting Kazakh and Afghanistani entities in a persistent campaign, with C2 servers active today (20/01/2026) that have been operating unreported since at least August 2022. 

During our tracking we identifed a Windows-based RAT, delivered as a DLL, that we have coined KazakRAT. KazakRAT allows the threat actor to download and run additional payloads, enumerate and collect host data, and search for and exfiltrate files. The malware has simple installation process, often containing decoys relevant to victims. The KazakRAT binaries themselves were not obfsucated making it simple to analyse. C2 communications are unencrypted and follow a simple beaconing mechanism over HTTP. We observed this same group also using modified version of XploitSpy, Android Spyware, for espionage activities targeting Uzbek and Syrian extremist groups.  

Due to an operational mistake by the adversary, we now own one of their Windows RAT domains, allowing us to redirect victim traffic to a sinkhole for local analysis and give visbility into live beaconing.  Given how straight forward the implant and protocol are, it is possible to re-implement the server-side C2 in Python, finishing the blue-team blog with a calc.exe POC :P 

<center>
  <img src="/assets/images/afguzb/2.png" width="1350" height="900" alt="Correlation Graph">
</center>
...
Like the above, the executable inside the msi is used to establish persistence via the creation of the Run key `8`. Noteably this sample had a decoy Word document, `flReport.doc`, that is a fake letter from the President of the Republic of Kazakstan. 
...
In this case, the `MY.DOC` is an actual Word Document. This is an analytical breifing/note about foreign economic activity (external trade) in the Karaganda Region in Kazakstan:
...
We can create a correlation graph to highlight overlaps in infrastructure across campaigns targeting disparate entities. Noteably, across all samples observed, we only observed C2 servers for KazakRAT located on IP addresses on AS209847 and AS52469.
We can create a correlation graph to highlight overlaps in infrastructure across campaigns targeting disparate entities. Noteably, across all samples observed, we only observed C2 servers for KazakRAT located on IP addresses on AS209847 and AS52469.
...
<center>
  <img src="/assets/images/afguzb/3.png" width="1350" height="900" alt="Correlation Graph">
</center>
...
We were unable to attribute these clusters of malware and associated C2 infrastructure, with high confidence, to any known threat group. However, based on the observed targeting and lure documents, we assess that this activity is likely linked to a state-affiliated actor leveraging modified open-source Android spyware alongside a Windows RAT, which we refer to as KazakRAT. The Windows implant is unobfuscated and straightforward to analyse, suggesting the operators were not primarily concerned with evasion and invested minimal effort in operational security (OPSEC) during development. Furthermore, the mistake of letting C2 domains expire may suggests this potential state-affiliated actor may lack sophistication.

This activity has been ongoing since at least August 2022, when the first Android APK was uploaded to VirusTotal, with the related C2 infrastructure remaining active at the time of writing. Over this period, we observed multiple KazatRAT variants containing differing command sets and primarily targeting victims in Kazakhstan. Based on the lures identified, the actor may be targeting employees in government or financial sector roles, particularly within the Karaganda region. This assessment is supported by multiple victim IP addresses observed via sinkhole telemetry that were geolocated to Karaganda. Additionally, we identified two KazatRAT samples delivered with lure content suggesting possible targeting of Afghan government employees.

In August 2022, Meta publised their [Quarterly Adverserial Threat Report](https://about.fb.com/wp-content/uploads/2022/08/Quarterly-Adversarial-Threat-Report-Q2-2022.pdf) in which it described actions taken against APT36, a Pakistan-based threat group also known as Transparent Tribe, reportedly targeting entities in Afghanistan, India, Pakistan, and Saudi Arabia. Meta noted that the group’s activity, while relatively low in sophistication, was persistent and targeted. We observed similar long-term, persistent targeting, including incremental changes across KazatRAT variants over time. These compilation dates indicate the campaign has likely been ongoing for more than three years. Meta also reported that APT36 used modified versions of XploitSpy, reinforcing a broader trend in which less sophisticated actors can successfully rely on open-source tooling rather than investing heavily in bespoke development.
...
| `MY.DOC`                                               | Decoy document (Afghani themed)| `d9e99210f813b0b265c3a5aa236128fe5cab5eb56da9a9551cd3f849d7b9405d` |
