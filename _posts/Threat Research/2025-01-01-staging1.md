...
Inspired by Censys' blog [ErrTraffic: Inside a GlitchFix Attack Panel](https://censys.com/blog/errtraffic-inside-glitchfix-attack-panel), we wanted to take a poke at the source-code but we had no idea how censys gained it.
...
During our investigation into ErrTraffic we discovered an authenticated file upload vulnerability that could be leveraged to upload a webshell and gain Remote Code Execution (RCE). In later versions we discovered the developers had patched this vulnerability, but we found another way. 
...
Unzipping the acquired panel we can see in the "panel" directory root there are two files `install.php` and `index.php`. 
...
The name of the malware is always choosen randomly and set in the `$storedName` variable. 
...
In ErrTraffic v2, there is literally no restrictions on file uploads - making our lives a lot easier! Using the in-built file upload feature, we can easily upload php webshells:
...
Whilst poking on the internet we discovered multiple threat actors exposing `/install.php` files that could've led to complete takeover and RCE. It appears, however, in some recent versions the installation process is locked down - requiring a pin to access the form to overwrite the configuration. 

However, with all of the source code we retrieved, this wasn't the case. We set up a ErrTraffic server that has the domain `sadrussianserver.xyz` and a rogue MySQL database on the domain `mysecretdatabase.com`. 

> When using a rouge database, after authenticating, you won't see the original victims/data. However, once authenticated and control of the database, it can faciliate further vulnerabilities like the RCE or path traversal
